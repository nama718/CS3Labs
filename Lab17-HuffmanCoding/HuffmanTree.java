...
public class HuffmanTree {
  ...
  void maketree(int[] count) {
    Queue<Node> PriorityQueue = new PriorityQueue<>((node1, node2) -> Integer.compare(node1.weight, node2.weight));
    for (int i = 0; i < count.length; ++i) {
      if (count[i] > 0)
        PriorityQueue.add(new Node((char) i, count[i]));
    }
    PriorityQueue.add(new Node(e, 1));
    while (PriorityQueue.size() > 1) {
      Node l = PriorityQueue.poll(), r = PriorityQueue.poll();
  ...
  void decode(BitInputStream input, String outfile) {
    try {
      PrintWriter p = new PrintWriter(new File(outfile));
      Node current = root;
      while(true) {
        int x = input.readBit();
        if(x == 1) current = current.right;
        else current = current.left;

        if((int)current.c == e) break;
        else if(current.c != sub) {
          p.write(current.c);
          current = root;
        }
      }
...
class BitOutputStream {
  ...
  private int numDigits;  // how many digits are currentrently in the buffer
