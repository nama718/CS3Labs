...
class MinHeapGeneric<T extends Comparable<T>> {
  ...
  private void doubleCapacity() {
    ...
    T[] heaptwo = (T[]) new Comparable[len];
  ...
  void siftDown(int index) {
    T left = heap[this.getLeftChildIndex(index)];
    T right = heap[this.getRightChildIndex(index)];
    T current = heap[index];
    int in = -1;
    if(left != null && current.compareTo(left) > 0 && (right == null || left.compareTo(right) <= 0))
      in = getLeftChildIndex(index);
    else if(right != null && current.compareTo(left) > 0 && (right == null || left.compareTo(right) >= 0))
      in = getLeftChildIndex(index);
    if(in != -1) {
      T sv = heap[in];
      heap[in] = heap[index];
