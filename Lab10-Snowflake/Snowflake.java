...
class SnowFlakePanel extends JPanel
{
    ...
    public int num(int limit) { 
        return (int)(Math.random() * (limit + 1)); 
    }
    public void frost(Graphics g, int num) {
        for(int i = 0; i < num; ++i) {
    ...
    public void drawSnowFlake(Graphics g, int cX, int cY, int lineLength) {
        ...
        g.drawLine(cX - dX, cY - dY, cX + dX, cY + dY);
        int[] x = {cX - dX, cX - dX, cX - dX * 2, cX + dX * 2, cX + dX, cX + dX};
        int[] y = {cY - dY, cY + dY, cY, cY, cY - dY, cY + dY};
        for(int i = 0; i < 6; ++i)
            drawSnowFlake(g, x[i], y[i], lineLength / 3);
